...
Agr1, Agr2, Agr3 - маршрутизаторы агрегации. Соединены друг с другом, Agr1 и Agr3 имеют резервные линки к ядрам.
...
Заставить генерировать трафик "клиента" не удалось, поэтому мак клиента не отображается. Либо это связано с особенностью образа, который я использовал вместо ASR.

В дальнейшем для каждого нового клиента необходимо будет просто добавить требуемые сабинтерфейсы на оборудование и xconnect по аналогии с уже имеющимися.
...
Также, так как у нас имеются кольца, на них можно поробовать перейти на erps.
...
Сеть управления сейчас завернута в сторону браса, но это ограничение обрезанной схемы. В идеале, если эта часть сети оператора, все управление будет уходить на общий сервер доступов и мониторинга.
